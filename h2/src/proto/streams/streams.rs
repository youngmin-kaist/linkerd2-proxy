...
fn drop_stream_ref(inner: &Mutex<Inner>, key: store::Key) {
    ...
    // The connection acts on a closed, unreferenced stream only when it is
    // deciding whether to close itself: when no other handle remains (`refs`
    // counts the connection's own) or a GOAWAY was sent or received. Any
    // other wake would poll the connection for nothing.
    if stream.ref_count == 0
        && stream.is_closed()
        && (me.refs <= 1 || actions.recv.is_going_away() || actions.send.is_going_away())
    {
