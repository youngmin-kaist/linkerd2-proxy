...
pub(super) struct Send {
    ...
    /// A GOAWAY was received: the connection closes once its streams end.
    going_away: bool,
...
impl Send {
    ...
    pub fn send_headers<B>(
        ...
        if counts.peer().is_local_init(frame.stream_id()) && !stream.is_pending_push {
            // A stream that may open now is opened here, on the caller's
            // thread, exactly as `pop_pending_open` opens it on the connection
            // task: counted, and send-ready, so `queue_frame` below schedules
            // it. Opening here lets the caller reserve send capacity for the
            // body before the connection task runs, so headers and body leave
            // in one pass instead of two. Nothing is waiting ahead of it, so
            // stream ids still open in order.
            if !self.prioritize.has_pending_open() && counts.can_inc_num_send_streams() {
