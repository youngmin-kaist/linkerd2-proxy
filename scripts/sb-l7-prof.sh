...
cleanup(){
  sudo -n pkill -x linkerd2-proxy 2>/dev/null; pkill -x mock-identity; pkill -x mock-policy; pkill -f "[m]ock-destinatio"
  timeout 20 ssh $HOST 'pkill -x dpumesh_v0_host; true' </dev/null >/dev/null 2>&1
}

step "0. clean (CORES=$CORES W=$W M=$M)"
cleanup; sleep 12
source scripts/dev-proxy-env.sh >/dev/null 2>&1
export LINKERD2_PROXY_DOCA_REP_PCI_ADDR=0b:00.1
...
DMESH_NUM_WORKERS=$W LINKERD2_PROXY_CORES=$RT_CORES \
    ...
    setsid sudo -n -E env $SHENV taskset -c $((16-CORES))-15 ${PROXY_BIN:-./target/release/linkerd2-proxy} > $LOG/dproxy.log 2>&1 </dev/null &
...
timeout 8 ssh $HOST "cd ~/bf-workspace && for j in \$(seq 0 $((M-1))); do setsid env DMESH_BACKEND_CONNECT=127.0.0.1:8086 DMESH_DST_IP=10.0.0.\$((1+j)) DMESH_DST_PORT=8086 DMESH_SERVER_IDX=\$((j % $W)) ./apps/dma_bench/build/dpumesh_v0_host -p 0b:00.1 -t 1 -d 1 > /tmp/ym_be_\$j.log 2>&1 </dev/null & done; exit 0" </dev/null >/dev/null 2>&1
...
timeout 8 ssh $HOST "cd ~/bf-workspace && for i in \$(seq 0 $((M-1))); do rm -f /tmp/ym_in_\$i.log; setsid env DMESH_PUSH_BRIDGE_PORT=\$((38080+i)) DMESH_DST_IP=10.0.0.\$((1+i)) DMESH_DST_PORT=8086 DMESH_SERVER_IDX=\$((i % $W)) ./apps/dma_bench/build/dpumesh_v0_host -p 0b:00.1 -t 1 -d 1 > /tmp/ym_in_\$i.log 2>&1 </dev/null & done; exit 0" </dev/null >/dev/null 2>&1
...
PID=$(pgrep -x linkerd2-proxy | head -1); step "4. h2load x$M (timed 16s, warmup 3s, -c1 -m300) + perf on pid $PID (window: t+5..t+13)"
...
(sleep 5; curl -s --max-time 5 http://127.0.0.1:4991/metrics | awk '/^request_total\{direction="outbound"/{s+=$NF} END{printf "%d", s}' > $LOG/req_t5.txt; sleep 8; curl -s --max-time 5 http://127.0.0.1:4991/metrics | awk '/^request_total\{direction="outbound"/{s+=$NF} END{printf "%d", s}' > $LOG/req_t13.txt) & PIDS="$PIDS $!"
timeout 120 ssh $HOST "for i in \$(seq 0 $((M-1))); do h2load --duration=16 --warm-up-time=3 -c1 -m300 http://127.0.0.1:\$((38080+i))/ok > /tmp/ym_h2_\$i.log 2>&1 & done; (sleep 6; mpstat -P ALL 8 1 | awk '/Average/ && \$2!=\"CPU\" {b+=100-\$NF} END{printf \"[host] busy cores during window ~= %.1f / 16\\n\", b/100}'; pidstat -u 8 1 -C 'h2load|dpumesh' | awk '/Average/ && \$NF!=\"Command\" {c[\$NF]+=\$8} END{for(k in c) printf \"[host] %s %.2f cores\\n\", k, c[k]/100}') & wait; python3 -c \"
import re,glob
t=0; n=0; ok=0
...
wait $PIDS
echo "[window] proxy requests in perf window: $(( $(cat $LOG/req_t13.txt) - $(cat $LOG/req_t5.txt) )) over 8s => $(( ( $(cat $LOG/req_t13.txt) - $(cat $LOG/req_t5.txt) ) / 8 )) req/s"
